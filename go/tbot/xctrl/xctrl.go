...
func Handler(bot *xbot.Bot, cmd *xproc.Cmd, loggingPatch ctxlog.PatchAttrs) http.HandlerFunc { //nolint:gocognit // reason to refactor
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			aw.L(ctx, fmt.Errorf("body reading: %w", err))
		}
		...
		case http.MethodGet:
			...
			if fileID == "" {
				...
				aw.L(ctx, fmt.Sprintf("%s %t %s", "ok/filePath", ok, filePath)) // TODO remove
		...
		case "RUN":
			...
			go func() { // TODO: limit concurrency
				...
				if req == nil { // TODO hmm... it happens?
					aw.L(ctx, "Script response skipped")
					return
				}
			...
			return
		default:
			aw.L(ctx, fmt.Errorf("method not allowed: "+r.Method))
